  ...
  try {
    ...
    if (!existingConfig.auth0CliClientId) {
      const { auth0Setup } = await inquirer.prompt({
        ...
        name: 'auth0Setup',
        message:
          'Have you set up your auth0 account (as describe in the Getting Started guide)? You should have a separate Auth0 Tenant for each developer and production.',
      });
    ...
    await spinOn(`Deploying Auth0 configuration...`, `Auth0 configuration deployed.`, async () => {
